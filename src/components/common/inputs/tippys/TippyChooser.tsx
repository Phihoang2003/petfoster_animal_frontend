...
export default function TippyChooser({
  title,
  data,
  styles,
  onValue,
          ...
          return (
            <ul
              ...
              className={classNames("shadow-lg overflow-hidden", {
                [styles?.classNamePopup ||
                "border border-gray-primary bg-white rounded-xl"]: true,
              })}
            >
              {data.map((item) => {
                return (
      ...
      >
        <div
          ...
          className={classNames(
            "flex items-center justify-between gap-2  whitespace-nowrap cursor-pointer w-full",
            {
              [styles?.minWidth || " min-w-[140px]"]: styles?.minWidth,
              [styles?.className ||
              "border border-[#333333] rounded-xl py-2 px-4"]: true,
            }
          )}
        >
