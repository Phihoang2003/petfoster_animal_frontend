    ...
    <>
      <TextareaAutosize
        ...
        onSelect={() => {
          if (
            !refMessage ||
            refMessage.current?.selectionStart ===
              refMessage.current?.selectionEnd ||
            !props.onSelected
          )
            return;
        ...
        className={classNames(
          "border-[#d0cfd4] border resize-none  bg-transparent p-3 outline-none focus:border-[#6366F1] hover:border-[#6366F1]",
          {
            [rounded]: true,
            [props.className || ""]: true,
            ["border-[#FA896B]"]:
              Validate.isNotBlank(props.message || "") && props.message,
          }
        )}
      />
      {Validate.isNotBlank(props.message || "") && (
        <small className="text-[#FA896B] ml-[14px] text-[0.8035714285714286rem] leading-[1.66]">
          {props.message}
        </small>
      )}
    </>
