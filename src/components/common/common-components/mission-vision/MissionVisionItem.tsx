...
export default function MissionVisionItem({
  data,
  positionHightlight = 12,
}: IMissionVisonItemProps) {
  return (
    <div className="w-full shadow-primary rounded-3xl px-[25px] py-[26px] text-black-main flex flex-col justify-between items-center gap-3">
      ...
      <p className="text-1xl">
        <b className="text-green-main-dark font-medium">
          {data.content.slice(0, positionHightlight)}
        </b>{" "}
        {data.content.slice(positionHightlight)}
      </p>
    </div>
  );
}
