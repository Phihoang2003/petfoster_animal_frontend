import { deleteReview, replyReview } from "@/apis/admin/review";
...
import NativeComfirm from "@/components/common/inputs/NativeConfirm";
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await replyReview({
        ...data,
        comment: refInput.current?.value,
      });
    ...
    <div className="text-black-main w-full max-w-full">
      <div
        className={classNames(
          "flex justify-between text-black-main border rounded-md",
          {
            "ml-5": option?.item,
          }
        )}
      >
            ...
            <div className="flex flex-col">
              ...
              {data.rating && data.sizes ? (
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-sm italic text-grey-secondary">
                    Size: {handleFormatSizes()}
                  </span>
                </div>
              ) : (
                ""
              ...
              <div className="flex items-center gap-2">
                {option?.delete && (
                  <span
                    ...
                    className="text-left text-red-primary hover:underline text-sm cursor-pointer mt-2 select-none"
                  >
                    Delete
                  </span>
                )}
        ...
        <div
          className={classNames(
            "flex flex-col  gap-3 pr-3 py-3 text-sm text-grey-secondary",
            {
          ...
          return (
            <Fragment key={comment.id + "replay" + index + comment.comment}>
              ...
              <Review
                ...
                data={{
                  ...comment,
                  avatar: option?.adminAvatar || contants.avartarAdminDefault,
                  name: option?.adminName || contants.shopName,
                }}
        ...
        {option?.replay && open && (
          <>
            ...
            >
              <div className="flex gap-3 items-start">
                <img
                  src={contants.avartarAdminDefault}
