...
import { ChatFooterContext } from "@/components/pages/chats/ChatFooter";
import { contants } from "@/utils/constant";
import { delay } from "@/utils/functionals";
import WrapperDialog from "@/components/dialogs/WrapperDialog";
import MapWraper from "@/components/boxs/maps/MapWrapper";
import Markers from "@/components/boxs/maps/Marker";
import Directions from "@/components/boxs/maps/Directions";
...
export default function MapDialog({ open, setOpen }: IMapDialogProps) {
  ...
  const [curAddressLocation, setCurAddressLocation] = useState<string | null>(
    null
  );
  ...
  const handleClickAddressItem = async (
  ...
  const handleSendMap = async () => {
    ...
    try {
      const res = await firebaseService.handleSendMap(
        conversationId,
        user?.username,
        data,
  ...
  useEffect(() => {
    (async () => {
      ...
      requestIdleCallback(() => {
        ...
        if (
          typeof dataChatItem.address === "string" &&
          dataChatItem.location?.yourLocation
        ) {
          ...
          setIniDataAddress({
            province: {
              ...province,
              ProvinceName: (dataChatItem.address as IAddress).province,
            } as IProvinceOutside,
            district: {
              ...district,
              DistrictName: (dataChatItem.address as IAddress).district,
            } as IDistrictOutside,
            ward: {
              ...ward,
              WardName: (dataChatItem.address as IAddress).ward,
            } as IWardOutside,
          });
        }
      });
        ...
        <div className="flex flex-col h-[76vh] w-full max-w-full overflow-hidden">
            ...
            >
              ...
              {coordes && (
                <Markers
                  point={coordes}
                  isShop={
                    Boolean(params && params.username && params.id) || false
                  }
                />
              )}
          ...
          <WrapperAnimation
            ...
            className={classNames(
              "absolute top-3 left-3 bg-white z-20 w-14 h-14 flex items-center justify-center rounded-full text-xl cursor-pointer",
              {}
            )}
          >
                      ...
                      <div className="flex flex-col items-center gap-5 w-full justify-start mt-5 px-4">
                        <ul className="flex flex-col gap-2 list-disc px-5 pr-1 w-full max-w-full">
                          <li className="text-xs italic text-gray-primary break-all">
                            For security and accuracy reasons, the exact address
                        ...
                        <Province
                          initData={
                            (iniDataAddress &&
                              iniDataAddress.province?.ProvinceName) ||
                            undefined
                          }
                          name="province"
                          onValue={async (value) => {
                            ...
                            try {
                              const response = await getDevisionDistrictes(
                                value
                              );
                        ...
                        <District
                          initData={
                            (iniDataAddress &&
                              iniDataAddress.district?.DistrictName) ||
                            undefined
                          }
                          name="district"
                        ...
                        <Wards
                          name="ward"
                          initData={
                            (iniDataAddress && iniDataAddress.ward?.WardName) ||
                            undefined
                          }
                    ...
                    {curAddressLocation && (
                      <div className="px-4 max-w-full mt-5 flex flex-col gap-3">
                        ...
                        <ul className="flex flex-col gap-2 list-disc px-5 w-full max-w-full">
                          <li className="text-xs italic text-gray-primary break-all">
                            The address displayed may not be accurate for your
                            location. This information is only for illustration
                            purpose
                          </li>
                          <li className="text-xs italic text-gray-primary">
                      ...
                      <div className="flex items-center justify-end text-xl gap-3">
                        <Tooltip placement="top" title={"Show direction"}>
                          <div
                            ...
                            className={classNames(
                              "cursor-pointer active:scale-95 transition-all ease-linear ",
                              {
                                ["text-black-main"]: validate() || !toDirection,
                                ["text-red-primary"]:
                                  !validate() && toDirection,
                              }
                            )}
                          >
                      ...
                      <div className="w-full  px-4">
                        {dataAddress.data.data.length > 0 ? (
                          dataAddress.data.data.map((address) => {
                            return (
                              <AddressItem
                                handleClick={handleClickAddressItem}
