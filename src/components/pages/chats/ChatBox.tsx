...
import { IConversation, IUserFirebase } from "@/configs/interface";
import ChatBody from "@/components/pages/chats/ChatBody";
import ChatFooter from "@/components/pages/chats/ChatFooter";
import ChatAnswer from "@/components/pages/chats/ChatAnswer";
...
export default function ChatBox(props: IChatBoxProps) {
  ...
  const [conversationSnapshot] = useCollection(
    firebaseService.queries.queryGetConversationForCurrentUser(user?.username)
  );

  const [userSnapshot] = useCollection(
    firebaseService.queries.getUserByUsername(user && user.username)
  );
  ...
  const handleClickChatNow = useCallback(async () => {
    ...
    const isConversationAlradyExists = conversationSnapshot?.docs.find(
      (conversation) => {
        return (conversation.data() as IConversation).users.includes(
          user.username
        );
      }
    );
    ...
    // set convertsation id
    const newConversation = await firebaseService.addConversation(
      user.username
    );
  ...
  const handleSendMessage = async (value: string, images?: ImageType[]) => {
    ...
    const newMessage = await firebaseService.handleSendMessageToUser(
      value,
      conversationId,
      user.username,
      { images }
    ...
    await firebaseService.setNewMessageConversation(
      conversationId,
      idNewMessage
    );
  };
      ...
      <Tippy
        ...
        render={(attr) => {
          return !showChatNow ? (
            ...
            <ChatAnswer
              handleClickChatNow={handleClickChatNow}
