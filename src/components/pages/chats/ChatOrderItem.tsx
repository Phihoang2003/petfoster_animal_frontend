        ...
        >
          <div
            className={classNames(
              "bg-white shadow-sm w-full relative overflow-hidden",
              {
                ["order-1"]: me,
                ["px-3 py-2 text-gray-primary italic rounded-xl text-1xl"]:
                  data.recall,
              ...
              <>
                  ...
                  <Link
                    ...
                    href={
                      user && contants.roles.manageRoles.includes(user.role)
                        ? links.adminFuntionsLink.orders.index +
                ...
                <div className="flex items-center gap-2 text-sm px-3 py-2 w-full">
                  <img
                    ...
                    src={dataOrder?.products[0].image}
                    alt={dataOrder?.products[0].image}
                  />
                  ...
                  <div className="w-full  flex-1">
                    <Link
                      ...
                      href={
                        user && contants.roles.manageRoles.includes(user.role)
                          ? links.adminFuntionsLink.product.index +
                            `/${dataOrder.products[0].productId}`
                          : links.products +
                            `${dataOrder.products[0].productId}/${stringToUrl(
                    ...
                    <div className="flex items-center gap-2">
                      <p className="text-red-primary">
                        {toCurrency(dataOrder.products[0].price)}
                      </p>
                      <del className="text-xs">
                        {toCurrency(
                          dataOrder.products[0].price +
                            (dataOrder.products[0].price * 8) / 100
                        )}
                      </del>
                    </div>
                  </div>
            ...
            {data.recall && (
              <p
                dangerouslySetInnerHTML={{
                  __html: "message has been recalled",
                }}
              ></p>
            )}
