...
import ChatItem from "@/components/pages/chats/ChatItem";
import ChatMapItem from "@/components/pages/chats/ChatMapItem";
import ChatOrderItem from "@/components/pages/chats/ChatOrderItem";
...
export default function ChatBody({ conversationId, open }: IChatBodyProps) {
  ...
  const queryMessages =
    firebaseService.queries.generateQueryGetMessages(conversationId);

  const [messageSnapshot, loading, error] = useCollection(queryMessages);
  if (error) {
  ...
  const messagesData = useMemo(() => {
    ...
    return messageSnapshot.docs.map((item) => {
      return {
        ...item.data(),
        id: item.id,
        sendAt: convertFirestoreTimestampToString(
          item.data().sendAt as Timestamp
        ),
      } as IMessage;
    });
  }, [messageSnapshot]);
      ...
      <div className="bg-[#F3F4F6] flex-1  w-full relative py-8 pb-0 px-5 flex flex-col gap-5 overflow-y-auto scroll hide-scroll scroll-smooth">
        {messagesData.map((item) => {
          ...
          if (item.type === "message") {
            return (
              <ChatItem
                styles={{ maxImageOnRow: 3, ojectFit: "contain" }}
                key={item.id}
                data={item}
                me={me}
              />
