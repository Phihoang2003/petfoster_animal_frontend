...
import PopupMessage from "@/components/pages/chats/PopupMessage";
    ...
    >
      <div className="flex items-start text-black-main text-sm max-w-[80%] gap-3">
        ...
        {me && user?.username !== data.currentUser && (
          <Link
            className="order-1"
            href={links.adminFuntionsLink.users.detail + user?.id}
          >
            <Avatar
              sx={{
                width: "34px",
        ...
        >
          ...
          >
            <div
              className={classNames(
                "py-2 px-3 rounded-xl max-w-[100%] break-all shadow-sm flex flex-col text-1xl gap-2",
                {
                  ["bg-white"]: true,
                  [" text-right"]: me,
                  ["order-1"]: me,
                ...
                <div
                  style={{
                    gridTemplateColumns: `repeat(${
                      data.images.length <= styles.maxImageOnRow
                        ? data.images.length
                        : styles.maxImageOnRow
                    }, minmax(0, 1fr))`,
                  }}
          ...
          <span className="text-xs px-2 text-[#8D8D8D] italic">
