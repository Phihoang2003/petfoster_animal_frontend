...
import OrderPopupItem from "@/components/pages/chats/components/OrderPopupItem";
...
export default function OrderPopup({
  ...
  const handleSendOrder = async (data: IOtherHistory) => {
    ...
    const newMessage = await firebaseService.handleSendOrder(
      conversationId,
      username,
      { orderId: data.id + "" }
    );
  ...
  return (
    <Tippy
      ...
      render={(attr) => {
        return (
          <div
            className="flex flex-col bg-white rounded-md shadow-lg  pb-3 text-black-main w-[calc(94%-48px)] select-none"
            tabIndex={-1}
            {...attr}
          >
            <div className="flex items-center justify-between px-3 py-2">
              ...
              <WrapperAnimation
                onClick={handleClose}
                className="flex items-center justify-center cursor-pointer top-0 right-0"
                hover={{}}
              >
