  ...
  return (
    <div
      ...
      className={classNames(
        ...
        {
          ["border-[3px] border-[#A1A2D3] rounded-[40px] min-h-[338px] min-w-[230px]"]:
            variant === "circle",
    ...
    >
      ...
      <div className="absolute top-0 left-0 p-4 text-white  flex items-center justify-center text-sm">
        <FontAwesomeIcon
          ...
          icon={data.containVideo ? faVideo : faImage}
        />
      </div>
      ...
      <AnimatePresence>
        {model && (
          <motion.div
            ...
            className={classNames(
              "absolute bg-[rgba(0,0,0,0.45)] inset-0 w-full h-full  flex flex-col items-center justify-between py-8 pt-4 px-4 text-white",
              {
          ...
          >
            ...
            <div className="flex items-center gap-2 w-full">
              ...
              <div className="flex flex-col w-full flex-1 max-w-full">
                <Link
                  href={links.users.profiles.personalpage + data.user.username}
                  className="truncate max-w-[70%] text-1xl font-medium hover:underline"
                >
                  {data.user.displayName || data.user.username}
                </Link>
                <p className="truncate  max-w-[70%] text-sm font-normal">
                  {data.title}
                </p>
              </div>
            </div>
