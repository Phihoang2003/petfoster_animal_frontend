    ...
    >
      <div
        className={classNames(
          "h-[33px] w-[33px] rounded-full text-lg flex items-center justify-center text-white",
          {
            ["bg-violet-primary"]: (value && value >= 10) || check,
            // remove !check
            ["bg-[#C2C2C2]"]: value && value < 100,
          }
        )}
      >
        {(value && value >= 100) || check ? (
...
function LineProgress({ progressNun }: ILineProPressProps) {
  return (
    <div className="mb-[60px] mt-[80px] px-[30px] hidden lg:flex items-center justify-between w-full gap-2">
      <div className="flex items-center justify-between flex-1 gap-2">
        {data.map((item, index) => {
          return (
            <_btn
              value={item.id < progressNun ? 100 : -1}
              key={item.id}
              data={item}
            />
          );
