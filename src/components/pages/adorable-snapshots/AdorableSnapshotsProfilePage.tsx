...
import { listTabsPostProfile } from "@/data/header";
...
export default function AdorableSnapshotsProfilePage({
  id,
}: IAdorableSnapshotsProfilePageProps) {
  // redux
  const { user } = useAppSelector((state: RootState) => state.userReducer);

  const [type, setType] = useState<string | undefined>("posts");
  ...
  const profileData = useQuery({
    ...
    queryFn: async () => {
      return await getUserWithUsername(id);
    },
  });
  ...
  return (
    <div className="w-full max-w-full">
      ...
      <div className="my-16 w-full flex flex-col gap-6">
        ...
        <div>
          <InfinityPosts username={id} type={type} />
        </div>
      </div>

      {openEdit && (
        <UpdatePostDialog
          onClose={() => setIdPost(null)}
          open={openEdit}
          setOpen={setOpenEdit}
        />
      )}
