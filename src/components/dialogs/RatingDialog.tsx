...
import { DetailOrderHistoryContext } from "@/components/pages/order-histories/DetailOrderHistoryPage";
...
export default function RatingDialog({
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    console.log(form, data.productId, context.data?.id);
    ...
    try {
      ...
      await firebaseService.publistRatingProductNotification(
        data,
        user?.username
      );
    } catch (error) {
  ...
  return (
    <WrapperDialog open={open} setOpen={setOpen}>
      <DialogTitle
        sx={{
          py: "20px",
        }}
      ...
      >
        <span className="text-cente text-[#989898] cursor-pointer">
          Maybe later
        </span>
      </DialogActions>
    </WrapperDialog>
