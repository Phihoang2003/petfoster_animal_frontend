...
function UpdatePostDialog({ open, setOpen, onClose }: IUpdatePostDialogProps) {
  ...
  const onDrop = useCallback(async (acceptedFiles: any) => {
    ...
    const visibleFiles = files.filter((item) => {
      if (validateMedia(item)) {
        messages.push(`${item.name} photo larger than 3MB`);
      }
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await updatePost(
        { title: refInput.current.value, medias: images },
        data.id as string
      );
  ...
  const validateImages = useCallback(
    (checkLastImage = true, image?: IMediasPrev) => {
      ...
      if (checkLastImage) {
        ...
        if (images.length && images.filter((item) => item.id).length <= 1)
          return true;
      }
  ...
  const setUpdateData = useCallback(async () => {
    ...
    requestIdleCallback(async () => {
      ...
      refInput.current.value = data.title;
      setLoadingFrom(false);
    });
    ...
    >
      ...
      >
        ...
        <div className="w-full mt-8 rounded-[20px] border border-gray-primary p-5 flex flex-col justify-between">
          ...
          <div className="flex items-center justify-start gap-5">
            ...
            <EmojiPicker
              classnNameIcon="text-violet-post-primary"
      ...
      {openModal.state && (
        <WrapperDialog
          ...
          setOpen={(v) => setOpenModal({ ...openModal, state: v })}
        >
          <div className="p-6 flex flex-col gap-4 items-center text-black-main">
            ...
            <div className="flex items-center justify-between text-sm">
              ...
              <WrapperAnimation
                onClick={
                  handleCloseImage ? () => handleCloseImage() : undefined
                }
                hover={{}}
