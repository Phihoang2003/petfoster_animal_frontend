...
export default function CustomReasonDialog({
  reasons,
  label = "Tell them the reason for the cancellation",
  handleAfterClickSend,
  onClose,
}: ICustomReasonDialogProps) {
  ...
  return (
    ...
    >
      <div className="py-6 px-8 relative overflow-hidden">
        <h4 className="text-xl text-[#303B4E] font-semibold text-center mb-5 uppercase">
          {label}
        </h4>
        ...
        >
          {reasons.map((item, index) => {
            return (
              <FormControlLabel
                onClick={() => handleChooseReason(item, index)}
                key={item}
                value={item}
                control={<Radio />}
                label={item}
          ...
          {error && (
            <span className="text-xs text-red-primary">
              {contants.messages.review.whenEmptyReason}
            </span>
          )}
        </RadioGroup>
        ...
        <WrapperAnimation
          onClick={handleClose}
          hover={{}}
          className="absolute top-0 right-0 flex items-center justify-center text-lg py-3 px-4 cursor-pointer"
        >
