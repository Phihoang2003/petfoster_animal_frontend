...
const Validate = {
  ...
  isValidBirthday(value: string, maxAge = 14): boolean {
    const age =
      new Date().getFullYear() - new Date(value).getFullYear() < maxAge;
  ...
  isValidString(value: string, prefix: string): ValidateType {
    if (this.isBlank(value))
      return { message: prefix + " can't be blank ", error: true };

    if (this.isNumber(value))
      return { message: prefix + " is not numberic ", error: true };
  ...
  description(value: string): ValidateType {
    if (this.isBlank(value))
      return { message: `Description can't be blank `, error: true };
    return { message: "", error: false };
  },
  ...
  message(value: string): ValidateType {
    if (this.isBlank(value))
      return { message: `Message can't be blank `, error: true };
    return { message: "", error: false };
  },

  number(value: string | number, name?: string): ValidateType {
    if (!this.isNumber(value.toString()))
      return { message: `${name} is invalid `, error: true };

    if (Number.parseInt(value.toString()) <= 0)
      return { message: `${name} is invalid `, error: true };
  ...
  infomation(
    ...
    if (typeof value === "object") {
      if (value.length <= 0)
        return { message: `${callName} can't be blank `, error: true };
    } else {
      if (this.isBlank(value))
        return { message: `${callName} can't be blank `, error: true };
      ...
      if (this.isSpecialChars(value))
        return {
          message: `${callName}  is not include special characters `,
          error: true,
        };
    }
  ...
  username(value: string, max = 16): ValidateType {
    if (this.isBlank(value))
      return { message: "Username can't be blank ", error: true };
  ...
  date(value: string): ValidateType {
    if (this.isBlank(value))
      return { message: "Date can't be blank ", error: true };

    const date = new Date(value);

    if (date < new Date())
      return {
        message: "Date must be greater than to the current date",
        error: true,
      };
  ...
  address(value: string, dataMatch?: any[]): ValidateType {
    if (this.isBlank(value))
      return { message: "Address can't be blank ", error: true };

    if (dataMatch) {
      if (dataMatch.length <= 0)
        return { message: "Address invalid ", error: true };
    }
  ...
  dropdownTippy(
    value: string,
    option: { dataMatch?: any[]; name?: string }
  ): ValidateType {
    if (this.isBlank(value))
      return { message: `${option?.name || ""} can't be blank `, error: true };

    if (option.dataMatch) {
      if (option.dataMatch.length <= 0)
        return { message: `${option?.name || ""} invalid `, error: true };
    }
  ...
  division<T>(value: string, dataMatch: T[], key: keyof T): ValidateType {
    ...
    if (!dataMatch.find((item) => item[key] === value))
      return { message: "Address invalid", error: true };
  ...
  birthday(value: string): ValidateType {
    if (this.isBlank(value))
      return { message: "Birthday can't be blank ", error: true };
    if (this.isValidBirthday(value))
      return {
        message: "You are not old enough to use the service ",
        error: true,
      };
  ...
  phone(value: string): ValidateType {
    if (this.isBlank(value))
      return { message: "Phone number can't be blank ", error: true };

    if (!this.isPhone(value))
      return { message: "Phone number invalid ", error: true };
    return { message: "", error: false };
  },
  ...
  review(value: IDataReview, option = { max: 100, min: 10 }): ValidateType {
    if (this.isBlank(value.content) || value.star === 0)
      return { message: contants.messages.review.whenEmpty, error: true };

    if (value.content.length < option.min || value.content.length > option.max)
      return {
        message: `Review min ${option.min} and max ${option.max}`,
        error: true,
      };
